...
extern double aplicar_mh(const double *, int, int, int, int, double, int *, int, int);
...
//	Check Number of Input Args
	if(argc < 8) {
		...
		fprintf(stderr,"  ./programa n m nGen tamPob m_rate n_hilos_ini n_hilos_fit\n");
		return(EXIT_FAILURE);
	}
	...
	int n_hilos_ini = atoi(argv[6]);
	int n_hilos_fit = atoi(argv[7]);
...
//	Call Metaheuristic
	double value = aplicar_mh(d, n, m, n_gen, tam_pob, m_rate, sol, n_hilos_ini, n_hilos_fit);
