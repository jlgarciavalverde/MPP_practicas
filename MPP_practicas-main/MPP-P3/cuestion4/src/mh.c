 ...
 */
void fitness(const double *d, Individuo *individuo, int n, int m, int n_hilos_fit)
{
    double suma = 0.0;

    #pragma omp parallel for num_threads(n_hilos_fit) reduction(+:suma) default(none) shared(d,individuo,n,m)
    for (int i = 0; i < m; i++)
    {
...
double aplicar_mh(const double *d, int n, int m, int n_gen, int tam_pob, double m_rate, int *sol, int n_hilos_ini, int n_hilos_fit)
{
    ...
    #pragma omp parallel for num_threads(n_hilos_ini)
    for (int i = 0; i < tam_pob; i++)
    {
        poblacion[i] = (Individuo *)malloc(sizeof(Individuo));
        poblacion[i]->array_int = crear_individuo(n, m);
        fitness(d, poblacion[i], n, m, n_hilos_fit);
    }

    #ifdef TIMEGenPobIni
		double tf = mseconds();
		printf("Tiempo bucle for_ini: %.2lf sec\n", (tf - ti)/1000);
	#endif
    ...
    for (g = 0; g < n_gen; g++)
    {
        ...
        for (int i = 0; i < tam_pob; i++)
        {
            fitness(d, poblacion[i], n, m, 1); // n_hilos_fit = 1
        }
