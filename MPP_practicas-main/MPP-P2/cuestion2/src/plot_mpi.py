...
# ==============================
SUMMARY_DIR = "resumenes_todos"
PLOTS_DIR = "plots_todos"
...
def plot_metric(data_dict, ylabel, title, filename):
    ...
    for (nem_val, ngm_val), pairs in data_dict.items():
        ...
        # --- Etiqueta en el gráfico al final de la línea ---
        if x_np and y_val:
            plt.text(
                x_np[-1] + 0.1,          # un poco a la derecha del último punto
                y_val[-1],               # misma altura
                f"NEM={nem_val}\nNGM={ngm_val}",
                fontsize=8,
